...
from pos.models import Company, OrderItem
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--company", type=int, help="Company ID to scope the import", required=True)
    ...
    def handle(self, *args, **options):
        ...
        try:
            company_id = options.get("company", None)
            if not company_id:
                raise CommandError("Company ID is required.")
            company = Company.objects.get(id=company_id)
        except Company.DoesNotExist:
            raise CommandError(f"Company with ID {company_id} does not exist.")

        headers = {"X-API-KEY": settings.DETRACK_API_KEY, "Content-Type": "application/json"}
        ...
        try:
            ...
            with open("/tmp/detrack_import.csv", "r") as f:
                OrderItem.handle_detrack_import(company, f)
